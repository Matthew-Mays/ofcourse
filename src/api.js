...
export const getCourses = () => {
  return fetch(PREFIX + "/courses").then(handleErrors).then((res) => res.json());
};

export const getLessons = (courseId) => {
  return fetch(PREFIX + "/lessons?courseId=" + courseId).then(handleErrors).then((res) =>
    res.json()
  );
};
...
export const destroyLesson = lesson => {
  return deleteData(PREFIX + `/lessons/${lesson.id}`, lesson)
}
